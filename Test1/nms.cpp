...
//		globals->physicalWindowSize({ 800, 600 });
	...
	// --------------------
	GLFWwindow* window = glfwCreateWindow(globals->physicalWindowSize().x, 
							globals->physicalWindowSize().y, "LearnOpenGL", NULL, NULL);
	if (window == NULL)
	{
...
//#define CIRCLES
	...
	shader.create(path + "thebookofshaders.v.glsl",
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// -----
		processInput(window);
		float cameraAspect = globals->physicalWindowSize().x / 
		...
		glm::mat4 projection = glm::perspective(cameraFieldOfView, cameraAspect, cameraNearClip, cameraFarClip);
		glm::mat4 model(1.0);
		glm::mat4 pvm(1.0);// = projection * view * model;
		glBindVertexArray(VAO);
		shader.use();
