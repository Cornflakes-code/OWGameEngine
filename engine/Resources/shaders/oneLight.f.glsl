...
void main()
{
    ...
    vec2 light1 = vec2(sin(u_time*1.2+50.0)*1.0 + cos(u_time*0.4+10.0)*0.6,sin(u_time*1.2+100.0)*0.8 + cos(u_time*0.2+20.0)*-0.2)*0.2;
    vec3 lightColor1 = vec3(particlecolor);
    
    
    float cloudIntensity1 = 0.7*(1.0-(2.5*distance(uv, light1)));
    float lighIntensity1 = 1.0/(100.0*distance(uv,light1));

	fragColor = vec4(vec3(cloudIntensity1)*lightColor1 + lighIntensity1*lightColor1, 1.0);
}