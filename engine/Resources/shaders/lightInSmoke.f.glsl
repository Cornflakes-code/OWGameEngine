...
void main()
{
	...
	vec2 uv =  UV.xy/u_resolution.x;
    
    vec2 center = vec2(0.5,0.5*(u_resolution.y/u_resolution.x));
    
    vec2 light1 = vec2(sin(u_time*1.2+50.0)*1.0 + cos(u_time*0.4+10.0)*0.6,sin(u_time*1.2+100.0)*0.8 + cos(u_time*0.2+20.0)*-0.2)*0.2+center;
    vec3 lightColor1 = vec3(1.0, 0.3, 0.3);
    
    vec2 light2 = vec2(sin(u_time+3.0)*-2.0,cos(u_time+7.0)*1.0)*0.2+center;
    vec3 lightColor2 = vec3(0.3, 1.0, 0.3);
    
    vec2 light3 = vec2(sin(u_time+3.0)*2.0,cos(u_time+14.0)*-1.0)*0.2+center;
