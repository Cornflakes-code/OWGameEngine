...
{
	int fontHeight = 12;
	mX = new TextBillboardDynamic("arial.ttf", fontHeight);
	mY = new TextBillboardFixed("arial.ttf", fontHeight);
	mZ = new TextBillboardFixed("arial.ttf", fontHeight);
	...
	//nice.first /= 2.0f;
	//nice.second /= 2.0f;
	mX->createText("X", nice.x, nice.y);
	mY->createText("Y", nice.x, nice.y);
	mZ->createText("Z", nice.x, nice.y);
	mPoints->setUp({ mLines[0] });

	mX->color({ 1.0, 0.0, 0.0, 1.0f });
	mY->color({ 0.0, 1.0, 0.0, 1.0f });
	mZ->color({ 0.0, 0.0, 1.0, 1.0f });
